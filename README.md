...
| 在手机上继续任务 | 内容随屏幕换行、单终端视图、本地输入框、辅助键条和字号设置 |
...
电脑默认使用“自动”：按“字号 × 缩放”把完整画面缩放到窗口内，尽量一次看全，需要小于 10 px 才放得下时改为固定字号并滑动查看。画面装不下时左下角会显示裁切徽标，点它或点「适应窗口」可切换到按窗口缩放。手机默认使用“固定字号”，保持可读字号并滑动查看。打开、刷新网页或已安装 PWA 都保留远端终端尺寸，旧的“自适应”偏好不会自动恢复。确实需要远端程序按网页宽度重新换行时，可在当前页面选择“自适应 · 调整远端尺寸”；它会影响同一 pane 的原生终端和其他窗口，可能造成裁切，刷新后恢复本地显示模式。切换回“自动”“适应窗口”或“固定字号”会释放尺寸控制；已发生裁切时，在原生终端调整一次窗口大小即可重新计算布局，无需重启任务。
