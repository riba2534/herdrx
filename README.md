...
电脑默认使用“自动”：按“字号 × 缩放”把完整画面缩放到窗口内，尽量一次看全，需要小于 10 px 才放得下时改为固定字号并滑动查看。画面装不下时左下角会显示裁切徽标，点它或点「适应窗口」可切换到按窗口缩放。手机默认使用“自适应”，保持可读字号，内容随屏幕宽度换行；横竖屏切换和键盘弹出时自动调整。自适应会调整当前远端终端的行列数，同一终端的电脑窗口也会看到重新换行的内容。需要保留远端尺寸时，可切换到“固定字号”并滑动查看；已调整的尺寸由下次终端布局更新决定，不在退出网页时强制恢复。
