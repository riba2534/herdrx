...
| 操作终端 | 切换工作区、Tab 与 Pane，使用分屏、历史滚动、中文输入和 Shift+Enter；手机默认本地输入框，点发送后整段提交一次 |
...
进入主机后，选择原有工作区和 Tab，在对应 Pane 中继续操作。电脑上使用分屏查看多个终端，也可打开「本地输入」在本地编辑后再整段发送。手机默认使用底部本地输入框：中文、粘贴和换行都在本地完成，点击「发送」后整段进入当前终端并提交一次；需要逐键操作时点「直接输入终端」或标题栏键盘按钮。底部辅助键仍可发送 Ctrl+C、Esc、Tab 和方向键。发送成功只表示输入已被接受，不代表远端程序已经执行完。电脑和手机的输入方式分开记住：缩小窗口不会把手机输入框留在电脑上，电脑上主动打开的本地输入也不会被窗口变化清掉。

电脑默认使用固定 14px 字号，完整画面超出时可滚动查看；需要一次看全当前行列时点「适应窗口」。手机默认使用“自适应”，保持可读字号，内容随屏幕宽度换行；横竖屏切换和键盘弹出时自动调整。自适应会调整当前远端终端的行列数，同一 Pane 的电脑窗口也会看到重新换行的内容。需要保留远端尺寸时，可切换到“原始画面”并滑动查看；已调整的尺寸由下次终端布局更新决定，不在退出网页时强制恢复。
