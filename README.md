...
进入主机后，选择原有工作区和标签页，在对应终端中继续操作。电脑上可拖动分屏分隔或用 Ctrl+B R 调整比例，Ctrl+B ? 查看快捷键；断线时顶部条幅可立即重连，不必整页刷新。手机默认使用底部本地输入框：中文、粘贴和换行都在本地完成，点击「发送」后整段进入当前终端并提交一次；需要逐键操作时点「直接输入终端」或标题栏键盘按钮。底部辅助键仍可发送 Ctrl+C、Esc、Tab 和方向键。终端工具里可打开「选择文本」或「复制屏幕」。发送成功只表示输入已被接受，不代表远端程序已经执行完。电脑和手机的输入方式分开记住：缩小窗口不会把手机输入框留在电脑上，电脑上主动打开的本地输入也不会被窗口变化清掉。
