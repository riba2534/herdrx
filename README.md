...
电脑默认使用固定 14px 字号，完整画面超出时可滚动查看；需要一次看全当前行列时点「适应窗口」。手机默认使用“自适应”，保持可读字号，内容随屏幕宽度换行；横竖屏切换和键盘弹出时自动调整。自适应会调整当前远端终端的行列数，同一终端的电脑窗口也会看到重新换行的内容。需要保留远端尺寸时，可切换到“原始画面”并滑动查看；已调整的尺寸由下次终端布局更新决定，不在退出网页时强制恢复。
