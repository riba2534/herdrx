...
电脑默认使用“自动”：按字号与缩放适应画面，需要小于 10 px 才能放下时保留可读字号并滑动查看；手机默认“固定字号”。“适应窗口”允许缩小字体看全画面。这些本地显示选项只改变当前页面。

打开、刷新、输入或重新打开 PWA 都保持远端任务尺寸。需要程序按当前窗口重新换行时，在终端工具的“任务尺寸”中选择“使用此窗口尺寸”，同一终端的其他窗口也会看到变化。界面显示实际行列数与控制状态；本站其他窗口已控制时，可确认“转到此窗口控制”。选择“保持远端尺寸”、切离终端、进入后台或断线会释放控制，回来后先观察；输入不需要取得尺寸控制。释放后的尺寸由 Herdr 自身决定，不会写回旧尺寸。
