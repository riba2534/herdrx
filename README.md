<p align="center">
  <img src="design-system/herdrx/brand/exported/icon-256.png" alt="herdrx Logo" width="128" />
</p>
...
| 操作终端 | 切换工作区、标签页与终端，使用分屏、历史滚动、中文输入和 Shift+Enter；手机默认本地输入框，点发送后整段提交一次 |
| 向 Agent 传图片 | 粘贴、拖入或上传 PNG/JPEG/WebP/GIF，单张最大 20 MB |
| 在手机上继续任务 | 单终端视图、本地输入框、辅助键条；终端按屏幕自动选择完整显示、铺满宽度或可读字号，双指捏合调整字号，弹出键盘不改变远端尺寸 |
...
1. 以运行 Herdr 的同一用户，在远程主机复制并执行页面中的 GitHub 一行 CLI 安装命令，自动识别架构和校验。
...
进入主机后，选择原有工作区和标签页，在对应终端中继续操作。电脑上可拖动分屏分隔或用 Ctrl+B R 调整比例，Ctrl+B ? 查看快捷键；断线时顶部条幅可立即重连，不必整页刷新。手机默认使用底部本地输入框：中文、粘贴和换行都在本地完成，点击「发送」后整段进入当前终端并提交一次；需要逐键操作时点「直接输入终端」或标题栏键盘按钮。平板等触屏设备在电脑布局下也可从标签栏的键盘按钮打开辅助键。底部辅助键与 orca 手机端同序：Esc、Tab、Enter、Shift+Tab、空格、退格、Del、方向键，以及 Ctrl+C/D/L/Z/R/A/E/W/U，退格、Del 和方向键可长按连发；「粘贴」只送正文不追加回车。终端工具里可打开「选择文本」或「复制屏幕」。发送成功只表示输入已被接受，不代表远端程序已经执行完。电脑和手机的输入方式分开记住：缩小窗口不会把手机输入框留在电脑上，电脑上主动打开的本地输入也不会被窗口变化清掉。

电脑上每个分屏标题栏、手机上顶栏都有 **Terminal / Chat** 开关，切换只改变网页视图，不重启终端或 Agent。Chat 从 Claude / Codex 的会话记录读取真实问答，用户在右、助手在左，支持 Markdown、代码复制和工具记录。进入后需手动选择会话记录；即使只有一个候选也不会自动认领。所选记录只用于查看，输入始终发送到当前终端。

会话读取支持本机和具备 Python 3 的 SSH 远程主机；Tailcat 受限执行环境和未支持的 Agent 会提示切回终端，不会用屏幕文本伪造对话。遇到权限确认、交互菜单等 TUI 操作，也请切回 Terminal。读取暂时失败可重试，网站重启后可能需要重新选择记录。

Chat 输入框中 **Enter 发送、Shift+Enter 换行**，中文输入法确认候选时不会发送。断线时禁用发送，结果不确定时不自动重发。终端、远端任务及观察流不依赖 Chat 的开关状态。

本地显示默认“自动”：先按字号上限完整显示远端画面；放不下时铺满宽度、上下滑动查看；字号需要小于 10 px（手机 9 px）时保留可读字号并四向滑动。电脑默认 14 px，手机默认 13 px，电脑布局、手机布局和手机横屏分别记忆。手机上终端画面右上角常驻一个角标，显示远端行列与控制状态；电脑上只在画面被裁切、由其他窗口控制或其他窗口释放后留下较小尺寸时出现。点开角标可切换“自动 / 完整显示 / 固定字号”、调整字号，手机上还可直接切到对话视图。手机和平板在终端画面上双指捏合即调整字号，松手后生效；捏到最小以下切换为完整显示，页面其他区域仍可使用浏览器缩放。这些本地显示选项只改变当前页面，不改变远端尺寸。

打开、刷新、输入或重新打开 PWA 都保持远端任务尺寸。需要程序按当前窗口重新换行时，在角标菜单或终端工具的“任务尺寸”中选择“使用此窗口尺寸”，菜单会先说明远端将从多大改为多大，列数少于 50 时提示表格和 diff 可能折行；同一终端的其他窗口也会看到变化，并在角标上显示“其他窗口控制”，可确认“转到此窗口控制”。控制期间旋转屏幕或调整字号会重新排版，弹出软键盘只遮住画面、不改变远端行数。结束时可选“恢复为原尺寸并释放”，或“释放控制”保持当前尺寸；切离终端、进入后台或断线也会释放控制，但不会自动恢复。其他窗口释放控制后如果留下的尺寸比当前窗口小很多，角标提示“较小”，可一键“按此窗口尺寸恢复”，调整一次后继续观察。输入不需要取得尺寸控制。

Chat 支持粘贴、拖入或选择图片：先显示可移除的缩略图并上传暂存，**不会立即向终端输入路径**；点发送后，文字和图片引用一起提交一次，也可以只发送图片。上传失败时先移除或重试失败附件。普通 Terminal 模式仍保留原有图片粘贴路径行为；Agent 是否能读取图片取决于其自身能力。

管理员配置语音网关后，Chat 显示麦克风按钮。首次使用需浏览器授权（HTTPS 或 localhost）；转写只追加到当前草稿，**不会自动发送或执行命令**。可选的语音回复来自独立语音助手，不代表终端 Agent。停止录音、离开 Chat 或退出登录会释放麦克风和连接。配置方法见 [语音网关](docs/voice-input.md)。
