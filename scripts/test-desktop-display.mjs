  ...
  for (const name of engines) {
    ...
    try {
      for (const c of cases) {
        ...
        await expect(f.page.getByRole('button', { name: '完整显示', exact: true })).toHaveAttribute('aria-pressed', 'false')
        ...
        await openPaneTools(f.page)
        await f.page.getByRole('button', { name: '完整显示', exact: true }).click()
        await expect.poll(async () => { const m = await metrics(f.page); return m.screenWidth <= m.width + 1 && m.screenHeight <= m.height + 1 }).toBe(true)
        await expect(f.page.getByRole('button', { name: '完整显示', exact: true })).toHaveAttribute('aria-pressed', 'true')
