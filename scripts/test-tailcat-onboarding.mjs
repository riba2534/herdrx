  ...
  for (const engine of process.env.HERDRX_TEST_ENGINES?.split(',') || ['chromium']) {
    ...
    try {
      for (const width of [1440, 768, 390, 320]) {
        ...
        assert.ok(await command.evaluate(el => el.clientHeight < 60 && getComputedStyle(el).whiteSpace === 'pre'), 'command must stay visually one line on every viewport')
        await expect(page.getByRole('button', { name: '复制下载安装命令', exact: true })).toBeVisible()
        await fit(page)
        ...
        assert.equal(await page.evaluate(() => getSelection().toString()), installCommand)
        await page.getByText('安装说明与手动下载', { exact: true }).click()
        ...
        await expect(page.getByLabel('启动后台服务', { exact: true })).toHaveText('~/.local/bin/herdrx setup && ~/.local/bin/herdrx status')
        ...
        await expect(page.getByLabel('生成绑定凭据', { exact: true })).toHaveText('~/.local/bin/herdrx connect --plain')
        ...
        await expect.poll(() => page.evaluate(() => sessionStorage.getItem('herdrx.enrollmentTask.member'))).toContain('task-fixture')
