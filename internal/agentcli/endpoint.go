...
func (s *IPCServer) handleRefreshEndpoint(w http.ResponseWriter, r *http.Request) {
	...
	result, err := s.refreshEndpoint(input.Region)
	if err != nil {
...
func (s *IPCServer) refreshEndpoint(region *tailcfg.DERPRegion) (refreshEndpointResult, error) {
	...
	now := time.Now()
	payload := tunnel.EndpointUpdate{Version: 1, AgentID: updated.SetupID, ControllerID: updated.Binding.ControllerID, BindingID: updated.Binding.BindingID, ClientNode: updated.Binding.FormalClientNode, Address: address, Revision: updated.EndpointVersion, CreatedAt: now.Unix(), ExpiresAt: now.Add(10 * time.Minute).Unix()}
