...
export function Modal({ title, busy = false, onClose, children, className = '', role = 'dialog', descriptionID, closeLabel = '关闭' }: { title: string; busy?: boolean; onClose: () => void; children: ReactNode; className?: string; role?: 'dialog' | 'alertdialog'; descriptionID?: string; closeLabel?: string }) {
  ...
  return <Dialog.Root open onOpenChange={(open) => { if (!open && !busy) onClose() }}><Dialog.Portal><Dialog.Overlay className={`modal-backdrop ${role === 'alertdialog' ? 'confirmation-backdrop' : ''}`}/>
    ...
    }} onCloseAutoFocus={(event) => { event.preventDefault(); if (previous.current?.isConnected) previous.current.focus() }} onEscapeKeyDown={(event) => { if (busy) event.preventDefault() }} onPointerDownOutside={(event) => { if (busy || role === 'alertdialog') event.preventDefault() }}>
      <div className="modal-header"><Dialog.Title asChild><h2>{title}</h2></Dialog.Title><Button type="button" className="icon-button" aria-label={closeLabel} disabled={busy} onClick={onClose}><X size={18}/></Button></div>
      {children}
