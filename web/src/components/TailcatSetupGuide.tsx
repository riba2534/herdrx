...
import { ArrowLeft, ArrowRight, Check, Copy, Download, ExternalLink } from 'lucide-react'
...
export function TailcatSetupGuide({ children, pending, resuming, onClose }: { children: ReactNode; pending: boolean; resuming: boolean; onClose: () => void }) {
  ...
  return <div className="tailcat-guide">
    <p className="setup-intro">先在远程主机安装 herdrx 并启动后台服务，再将主机绑定到工作台。所有命令都在远程主机执行。</p>
    ...
    {step === 0 && <div className="form-stack">
      ...
      {checking ? <p className="field-hint" role="status">正在检查可下载的 CLI 版本…</p> : version ? <>
        ...
        <details className="setup-details"><summary>手动下载与安装</summary><p className="field-hint">按 CPU 选择压缩包，并下载 SHA256SUMS。解压后将 herdrx 放入 PATH。</p><div className="setup-download"><a href={`${download}/herdrx-linux-amd64.tar.gz`}>Linux x86_64</a><a href={`${download}/herdrx-linux-arm64.tar.gz`}>Linux ARM64</a><a href={`${download}/SHA256SUMS`}>SHA256SUMS</a></div><p className="field-hint">Release 附带 README-CLI.md，包含手动校验及安装的完整命令。</p></details>
      </> : <div className="notice notice-info" role="status"><p>{release?.status === 'unpublished' ? 'CLI 安装包尚未发布。请等待管理员发布首个 GitHub Release；已有 CLI 的主机可继续下一步。' : '暂时无法获取版本。请打开 GitHub Releases 查看可用版本，并按附件 README-CLI.md 安装；已有 CLI 的主机可继续下一步。'}</p><Button type="button" className="button-ghost" onClick={() => { heading.current?.focus(); setChecking(true); setRetry((value) => value + 1) }}>重新检查</Button></div>}
    </div>}
    {step === 1 && <div className="form-stack">
      ...
      <Command title="启动后台服务" value={'export PATH="$HOME/.local/bin:$PATH"\nherdrx setup && herdrx status'}/>
      ...
      <details className="setup-details"><summary>启动失败或没有 systemd</summary><p className="field-hint">运行 <code>herdrx doctor</code> 检查 Herdr，运行 <code>herdrx logs -n 100</code> 查看服务日志。找不到 Herdr 时可用 <code>herdrx setup --herdr-bin /path/to/herdr</code> 指定路径。</p><p className="field-hint">没有 systemd 用户会话时，可执行 <code>herdrx setup --skip-service</code> 后运行 <code>herdrx serve</code>，并交给你自己的进程管理器保活；直接关闭该终端会中断 Tailcat 访问。也可以返回选择 SSH 接入。</p></details>
    </div>}
    {step === 2 && <div className="form-stack">
      <p className="field-hint">后台服务就绪后，生成一次性绑定凭据并粘贴到下方。绑定完成会自动进入工作台。</p>
      <Command title="生成绑定凭据" value="herdrx connect --plain"/>
      {children}
      <details className="setup-details"><summary>凭据过期或连接失败</summary><p className="field-hint">重新运行 <code>herdrx connect --plain</code> 获取有效凭据；要作废尚未使用的旧凭据，请执行 <code>herdrx connect --renew --plain</code>。通过 <code>herdrx status</code>、<code>herdrx doctor</code>、<code>herdrx logs -n 100</code> 检查服务和网络。</p></details>
    </div>}
