...
const modeHint = { fit: '当前为适应窗口，完整显示当前终端', fixed: '当前为固定字号，超出部分可滚动', responsive: '当前为自适应，内容随窗口换行' } as const
...
export function DisplayToolbar({ display, actualFontSize, onChange, onSettings }: {
  ...
  return <div className="display-toolbar" role="toolbar" aria-label="终端显示" data-display-mode={display.mode}>
    <Button className="tool-button display-font" onClick={onSettings} data-tooltip={`${modeHint[display.mode]} · 点击调整字号`} aria-label="调整终端字号">Aa <span>{actualFontSize.toFixed(1).replace(/\.0$/, '')} px</span></Button>
    ...
    <Button className={`tool-button display-fit ${fitting ? 'tool-button-active' : ''}`} aria-pressed={fitting} data-tooltip={fitting ? '恢复固定字号，超出部分可滚动查看' : '缩放到完整显示当前终端'} onClick={() => onChange(fitting ? { mode: 'fixed', zoom: 100 } : { mode: 'fit', zoom: 100 })}><Scan size={14}/>适应窗口</Button>
  </div>
}
...
export function DisplaySettings({ display, mobile, onChange, onReset }: {
  ...
  return <fieldset className="display-settings">
    <legend>字号与缩放</legend>
    <p className="field-hint">自动保存在此浏览器，电脑和手机布局分别记忆。电脑默认固定 14px，超出可滚动；点“适应窗口”可一次看全。</p>
