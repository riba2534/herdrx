...
const modeHint = { auto: '当前为自动，尽量完整显示当前终端', fit: '当前为完整显示，完整显示当前终端', fixed: '当前为固定字号，超出部分可滚动' } as const
...
export function DisplaySettings({ display, mobile, onChange, onReset }: {
  ...
  return <fieldset className="display-settings">
    ...
    <p className="field-hint">{display.mode === 'auto' ? '自动模式以“字号 × 缩放”为上限缩小字号，尽量完整显示当前终端；缩到 10 px 仍放不下时，改为固定字号并可滑动查看；拖动字号滑杆会切换到固定字号。' : <>放大后可{mobile ? '单指滑动' : '使用滚动条或触控板'}查看完整画面。浏览器的页面缩放和双指缩放仍可使用。</>}</p>
