...
export function HostsPage() {
  ...
  const enrollError = useRef<HTMLSpanElement | null>(null)
  const enrollmentStartedAt = useRef(0)
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (!saved.id) return
      enrollmentStartedAt.current = saved.startedAt || Date.now()
      setDraft((current) => ({ ...current, transport: 'tailcat' }))
      setShowAdd(true)
  ...
  useEffect(() => {
    if (!enrollmentTaskID) return
    let cancelled = false
    const startedAt = enrollmentStartedAt.current || Date.now()
    setPending(true)
    setEnrollmentProgress('正在读取配对状态…')
    const run = async () => {
      try {
        for (;;) {
          if (cancelled) return
          const elapsed = Date.now() - startedAt
          const task = await api.getTailcatEnrollment(enrollmentTaskID)
          if (cancelled) return
          setEnrollmentProgress(enrollmentLabels[task.status] || task.status)
          ...
          if (task.status === 'failed') {
            sessionStorage.removeItem(TASK_KEY)
            setEnrollmentTaskID('')
            setFormError(enrollmentErrorMessage(new Error(task.error || '导入失败，请检查连接串是否有效')))
            setEnrollmentProgress(null)
            return
          }
          if (elapsed >= 64_000) {
            setTimeoutNotice('配对仍在进行。请在远程主机执行 ~/.local/bin/herdrx status 或 ~/.local/bin/herdrx logs 检查。稍后可刷新本页继续查看，无需重新粘贴连接串。')
            setEnrollmentProgress(null)
            return
          }
  ...
  }, [enrollmentTaskID, TASK_KEY])

  useEffect(() => {
    if (!enrollmentTaskID) {
      setEnrollmentElapsed(0)
      return
    }
    const tick = () => setEnrollmentElapsed(Math.max(0, Math.floor((Date.now() - (enrollmentStartedAt.current || Date.now())) / 1000)))
    tick()
  ...
  useEffect(() => {
    if (!formError) return
    enrollError.current?.scrollIntoView({ block: 'nearest' })
    connInput.current?.scrollIntoView({ block: 'nearest' })
  ...
  const submit = async (event: FormEvent) => {
    ...
    try {
      if (draft.transport === 'tailcat') {
        ...
        try {
          ...
          enrollmentStartedAt.current = Date.now()
          sessionStorage.setItem(TASK_KEY, JSON.stringify({ id: taskId, agent_id: res.agent_id, startedAt: enrollmentStartedAt.current }))
    ...
    finally { if (draft.transport !== 'tailcat') setPending(false) }
  }
  ...
  const visibleHosts = hosts.filter((host) => {
    if (!hostInFolder(host, selectedFolder, folders)) return false
  ...
  const openAdd = (trigger: HTMLButtonElement) => {
    ...
    if (waiting) { setShowAdd(true); return }
    setEditingHost(null)
    setDraft({ ...emptyDraft, folder_id: selectedFolder === '*' ? '' : selectedFolder, ssh_key_id: keys[0]?.id || '' })
    setFormError('')
    setShowAdd(true)
  }
  ...
  return <div className="app-shell">
    ...
    <main className="page page-hosts">
      ...
      {!loading && !error && hosts.length > 0 && <div className="host-list-toolbar"><span>{selectedFolder === '*' ? '全部主机' : selectedFolder === '' ? '未分组' : options.find((folder) => folder.id === selectedFolder)?.path} · {visibleHosts.length} 台</span><label className="host-search"><Search size={16} aria-hidden="true"/><Input aria-label="搜索主机" placeholder="搜索名称、地址、用户或文件夹" value={query} onChange={(event) => setQuery(event.target.value)}/></label></div>}
      {loading ? <div role="status"><p className="skeleton-status"><LoaderCircle className="spin" size={16} aria-hidden="true"/>正在读取主机…</p><div className="skeleton-grid"><i/><i/><i/></div></div> : error ?
        <EmptyState icon={<Server size={32} />} title="无法读取主机列表，请检查网络后重试" detail={error} action={<Button className="button-primary" onClick={() => void load()}>重试</Button>} /> :
        hosts.length === 0 ?
        ...
        visibleHosts.length === 0 ? <div className="host-search-empty"><h2>{query ? '没有匹配的主机' : '此文件夹还没有主机'}</h2><p>{query ? '试试其他名称、用户或文件夹。' : '添加主机，或从其他文件夹移入主机。'}</p>{query && <Button className="button-secondary" onClick={() => setQuery('')}>清除搜索</Button>}</div> :
    ...
    {showAdd && <Modal title={editingHost ? 'SSH 连接设置' : '添加主机'} busy={pending} allowCloseWhileBusy={draft.transport === 'tailcat'} onClose={draft.transport === 'tailcat' && (pending || waiting) ? dismissAdd : closeAdd} className={draft.transport === 'tailcat' ? 'tailcat-modal' : ''}>
        <Form onSubmit={submit} className="form-stack">
          ...
          {draft.transport === 'tailcat' && <TailcatSetupGuide pending={pending} resuming={waiting || Boolean(enrollmentProgress)} onClose={draft.transport === 'tailcat' && (pending || waiting) ? dismissAdd : closeAdd} onDefer={dismissAdd}>
            <label className="field"><span className="field-label">绑定凭据（herdrx://v1/…）</span><Textarea aria-label="绑定凭据（herdrx://v1/…）" ref={connInput} className={formError ? 'input textarea input-error' : 'input textarea'} value={connectionString} onChange={(event) => setConnectionString(event.target.value)} placeholder="粘贴 herdrx connect --plain 生成的完整内容" rows={3} required disabled={pending} autoComplete="off" autoCapitalize="none" autoCorrect="off" spellCheck={false} aria-invalid={Boolean(formError)} aria-describedby={formError ? 'tailcat-enroll-error' : 'tailcat-enroll-hint'} /></label>
            <small id="tailcat-enroll-hint" className="field-hint">凭据 10 分钟内有效，只能绑定一次，请勿分享。刷新页面可继续未完成的绑定。</small>
            {enrollmentProgress && <div className="notice notice-info" role="status" aria-live="polite"><span className="spinner-inline" />{enrollmentProgress}{enrollmentElapsed > 0 ? ` 已等待 ${enrollmentElapsed} 秒` : ''}{enrollmentElapsed >= 20 && <p>若长时间无进展，请在远程主机执行 ~/.local/bin/herdrx status 或 ~/.local/bin/herdrx logs 检查。</p>}</div>}
            {formError && <span id="tailcat-enroll-error" className="field-error" role="alert" ref={enrollError}>{formError}</span>}
