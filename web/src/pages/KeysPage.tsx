...
import { ArrowLeft, Copy, KeyRound, LoaderCircle, Pencil, Plus, Search, Trash2 } from 'lucide-react'
...
export function KeysPage() {
  ...
  return <div className="app-shell">
    ...
    <main className="page page-keys">
      ...
      {!loading && !error && <div className="host-list-toolbar"><span>{keys.length} 把密钥</span><label className="host-search"><Search size={16}/><Input aria-label="搜索密钥" placeholder="搜索名称或指纹" value={query} onChange={(event) => setQuery(event.target.value)}/></label></div>}
      {loading ? <div role="status"><p className="skeleton-status"><LoaderCircle className="spin" size={16} aria-hidden="true"/>正在读取密钥…</p><div className="skeleton-grid"><i/><i/></div></div> : error ? <EmptyState icon={<KeyRound size={32}/>} title="无法读取密钥列表，请检查网络后重试" detail={error} action={<Button className="button-primary" onClick={() => void load()}>重试</Button>}/> : keys.length === 0 ? <EmptyState icon={<KeyRound size={32}/>} title="还没有密钥" detail="导入已有私钥，或生成一把新的 Ed25519 密钥。" action={<Button className="button-primary" onClick={() => edit()}>添加第一把密钥</Button>}/> : filtered.length === 0 ? <div className="host-search-empty"><h2>没有匹配的密钥</h2><Button className="button-secondary" onClick={() => setQuery('')}>清除搜索</Button></div> : <div className="key-list">{filtered.map((key) => <article className="key-card" key={key.id}>
