...
后续完整 CI 在 WebKit 的 PWA 断网恢复用例中出现一次 30 秒内未返回登录页的失败。本机 17 轮插桩复测未复现，不能将其唯一原因归结为 socket 复用或后台页面。

审查确认并补测了认证检查中的独立缺陷：原先兜底超时不主动取消 fetch，手动重连在已有检查时被忽略，卸载后的挂起检查会妨碍 Strict Mode 重新挂载。现在每次检查使用可取消的 AbortController，超时与卸载均取消请求；手动重连替换旧检查，自动事件仍合并请求。API 在读取响应后检查取消状态，迟到响应不能恢复或撤销登录。

新增回归在原代码上产生 4 项失败，修复后全部通过。修复后前端共 532 项测试、类型检查、lint、构建、真实网站认证回归以及 Chromium/Firefox/WebKit PWA 回归通过。PWA 门禁保留原有等待时间与断网场景，并增加失败时的请求/服务端接收/响应记录，便于区分浏览器网络挂起和认证逻辑错误。最终集成 CI 以 [PR #12 的检查结果](https://github.com/riba2534/herdrx/pull/12/checks) 为准。
