...
## System OpenSSH 更新与回退

新增 `system_ssh` 认证无需数据库 schema 迁移，既有密码/密钥和 Tailcat 主机保持原路径。启用前准备服务账号的 OpenSSH、SSH 配置、known_hosts 与有效 Kerberos ticket；保留 `HERDRX_SSH_BIN` 及运行环境配置。更换系统用户、迁移到容器或重启后都应重新验证，不把网站数据库备份当作 Kerberos 身份备份。

回退到不支持 System OpenSSH 的版本时，此类主机记录会连接失败；其他认证方式不受影响。不要为了回退伪造网站密码或修改票据。先保留完整数据备份，修复部署环境或恢复支持该模式的网站版本。网站停止只断开访问，不结束远程任务。
...
herdrx update --version v0.1.0-rc.2    # 明确选择预发布或固定版本
...
CLI v0.1.0-rc.2 支持从网页命令优先选择工作台自带中继。先更新网站，再更新远程 CLI，通过原主机卡片「更新连接端点」复制命令并导入输出包，原主机、身份与授权保持不变。直接更新程序不会擅自迁移已有中继。新版命令、诊断身份和更新包需配套新版网站；迁移后恢复旧版程序前，先在新版中显式迁回原中继，不要回退身份或端点版本。
