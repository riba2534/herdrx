...
更新网站后刷新浏览器标签页或重新打开已安装 PWA。旧版保存的“自适应”转换为电脑“自动”或手机“固定字号”，保留字号和缩放。普通观察与输入保持兼容；旧页面请求旧式尺寸控制会收到刷新提示，旧二进制 resize 不改变远程任务。新页面连接旧网站时隐藏尺寸控制入口，继续观察。

本次更新不增加数据库迁移，尺寸控制只保存在工作台内存中。回退时保留当前数据与身份，不恢复历史数据库；网站停止仅断开观察与控制访问，远程 Herdr 和任务继续运行。重连不会自动恢复尺寸控制或重放输入。尺寸控制通过页面与网站能力协商启用，回退后刷新页面即可重新协商。

## 跨设备工作台位置

网站启动时在现有 schema 3 中幂等创建 `user_workbench_sessions` 和 `user_workbench_writers`，不改写已有账号、主机、凭据或远程会话。前者保存每个用户最后的浏览位置，后者保存每个 Web 登录会话下各浏览器页面的写入序号，防止慢请求覆盖较新的位置。序号与位置在同一事务中提交；退出或撤销 Web 登录会话时，对应序号记录级联删除，最后浏览位置保留。后续保存位置时也会清理该用户已过期登录的序号记录，过期登录不能重新写入。

该迁移只增加表，schema 版本保持 3；旧版 schema 3 网站可继续读取原数据，但不会提供新增的位置恢复及乱序写入保护。回退后应刷新浏览器，避免继续运行新版本缓存页面。更新前仍需备份完整数据目录，并用副本启动候选版本验证迁移。位置请求超过 5 秒会取消，恢复页面最多等待当前保存 1 秒后采用本页的最新位置；未成功保存的位置可能无法在另一设备恢复。

## Chat 媒体配置保留

启用语音的实例重建时需保留 `--env-file ./voice.env`（Compose 对应 `env_file`）及其权限；未注入语音变量会隐藏麦克风入口。图片无需新增数据库迁移。回滚只替换网站镜像，不删除图片暂存、账号或远程会话；备份环境文件可能含网关密钥，按敏感配置保管。具体变量见 [媒体输入说明](voice-input.md)。
