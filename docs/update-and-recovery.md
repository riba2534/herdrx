...
更新网站后刷新浏览器标签页或重新打开已安装 PWA。旧版保存的“自适应”会转换为电脑“自动”或手机“固定字号”，保留字号和缩放。仍运行旧资源的页面重连后，后端将其旧式自适应请求按观察连接处理，不授予尺寸控制。主动调整远端尺寸需在新版页面重新选择，且不跨页面加载恢复。已有裁切可通过调整一次原生终端窗口大小恢复，不需升级、停止 Herdr 或重启任务。
