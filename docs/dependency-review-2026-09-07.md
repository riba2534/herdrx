...
`sbom.cdx.json` 使用 CycloneDX 1.6，列出两个 Linux 架构中 CLI/网站实际导入的 Go 模块，以及前端安装依赖（含构建和测试工具）。前端成分表与生成机器的 CPU 架构无关：锁文件里带 `os`/`cpu`/`libc` 约束的可选二进制包（esbuild、Rollup、oxlint 等按平台分发的预编译产物）不逐平台登记，排除集合由 `web/pnpm-lock.yaml` 决定并记在 `herdrx:npm:platform-packages-excluded` 属性里；它们的许可原文与各自父包相同，父包已登记。这样 amd64 与 arm64 上生成的结果逐字节一致，`--check` 不再只在一种架构上通过。它是源码依赖清单，不是精确到符号的二进制组成证明，也不覆盖运行环境的操作系统包。发行附件的版本、提交和摘要由 release.json 与 SHA256SUMS 记录。
...
- `react-remove-scroll-bar 2.3.8` 和 `stackback 0.0.2` 的 npm 包声明 MIT，但未附独立许可文件。保留真实 package.json 的名称、版本、许可、作者和仓库声明，未伪造作者版权原文；`stackback` 内嵌 V8 格式化代码另有 BSD-3-Clause 头部，已完整保留。此项是分发材料缺口记录，不能将元数据声称为不存在的上游许可文件。
- esbuild、Rollup 和 oxlint 的平台子包使用对应同版本父包附带的许可，因此不逐平台登记（见上）。构建工具的本地原生包未被复制到网站运行镜像。
- `@napi-rs/lzma-linux-x64-gnu 1.5.1` 声明 MIT 且未附独立许可文件，同时它只在 x64 上安装（锁文件中它没有非平台的父包，且没有被任何包引用），登记它会让声明文件随生成机器架构变化。因此它与其他平台二进制一样被排除，不再出现在成分表与声明正文中；本行是这条排除的记录，不冒充它的上游许可原文。
