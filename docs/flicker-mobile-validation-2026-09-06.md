...
- 本轮远程尺寸读取验收在 Linux 上完成；后续增加的 macOS 本机与 Tailcat 路径见[尺寸读取验收](macos-terminal-geometry-validation-2026-09-17.md)。基础 SSH 的远端尺寸读取仍要求 Linux 和 Python 3，Tailcat 需更新配套 CLI。要求与错误排查见[安装说明](install.md#终端滚动兼容要求)。
