...
本页记录 `v0.1.0-rc.1` 发布前的本地验收范围。公开构建和分发结果以 [GitHub Actions](https://github.com/riba2534/herdrx/actions) 与 [对应 Release](https://github.com/riba2534/herdrx/releases/tag/v0.1.0-rc.1) 为准；正式版条件见 [发版清单](release-readiness-2026-09-06.md)。
...
公开发行还需通过同一提交的完整 GitHub CI、ZOT 实际分发、双架构原生 CLI 和公开 Release 下载检查；这些结果由流水线与版本记录单独提供，不能从上面的本地测试推断。项目源码采用 [MIT License](../LICENSE)，第三方依赖保留各自声明。

独立用户按文档操作、不同 NAT/运营商故障矩阵、真实手机操作，以及 24 小时关闭浏览器和 72 小时运行记录仍属于正式版待完成条件。不能将原生 guest、回环中继或短时容器测试当作这些项目已经通过。

RC 可先提供公开附件；正式版需继续关闭上述适用条件。每个候选的提交、命令、退出码、产物摘要和测试日志应保存为独立验收记录，不能仅凭此滚动文档判断某个下载文件已通过。
