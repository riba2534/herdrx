...
> 2026-09-25 更新：手机默认改为 `auto` 13px；`auto` 增加“铺满宽度、上下滑动”一档，手机可读下限为 9px；手机竖屏与横屏分别保存偏好；触屏捏合调整本地字号；角标菜单承载本地显示与任务尺寸操作。详见 [移动端缩放与尺寸控制](../mobile-scaling-2026-09-25.md)。
...
| 此窗口已控制尺寸，旋转或调整字号 | 按稳定后的可用区域计算新网格 |
| 此窗口已控制尺寸，触屏正在输入时弹出键盘或候选栏 | 保持行列与字号，画面滚动到光标（2026-09-25 起） |
| 用户选择“恢复为原尺寸并释放” | 先改回接管前的网格，观察到后再释放；被动释放仍不恢复（2026-09-25 起） |
...
客户端一次测量同一布局时刻的 viewport 与实际字体 metrics；扣除工具栏、安全区域和软键盘占用后计算可用空间。`ResizeObserver` 与 `visualViewport` 合并调度；复用当前字体就绪逻辑与 resize 合并（2026-09-25 起为 250ms），连续变化只保留最后一个目标，行列相同则不重复发出。
