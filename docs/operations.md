...
手机默认以可读字号调整当前终端的远端行列数，内容随屏幕宽度换行。字号、横竖屏和软键盘视口变化都会更新尺寸。同一 pane 的其他客户端共享这个远端尺寸；需要保留源布局时可切换到“固定字号”。退出网页不会重建终端，也不强制恢复之前的行列数。

提示“此终端正在其他窗口自适应显示”时，关闭另一个控制窗口后重连，或切换为“固定字号”继续查看。工作台不会自动抢占正在连接的控制客户端。
