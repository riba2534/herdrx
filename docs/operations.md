...
需要远端程序按网页宽度重新换行时，在终端工具的“任务尺寸”选择“使用此窗口尺寸”。收到远程确认后才显示由此窗口控制；实际行列数来自远端画面。该操作影响同一终端的原生 Herdr 与其他网页。尺寸控制与本地显示分开，调整字号不会自动释放控制，需选择“保持远端尺寸”。

另一个本站窗口控制时，可使用自绘确认框交接。工作台只释放自己已登记的旧连接；外部控制连接冲突时请先在对应客户端释放，网站不会强制驱逐。页面进入后台、切出终端、转聊天、退出、断线会释放控制；工作台运行时，前台每 5 秒续期，20 秒未续期开始释放。工作台整机离线时远程访问连接的断开检测可能更久，此期限不是远程任务的保活要求。

释放后按 Herdr 当前仲裁显示尺寸，不写回旧尺寸；原生窗口必要时可调整一次大小重新布局。观察与控制期间都可输入。应用滚轮会复用本站已有尺寸控制连接；没有长期控制时仍使用保留四维几何的短连接，不用历史偏移冒充鼠标滚轮。

## Herdr 版本与能力诊断

主机详情的“Herdr 兼容性”显示已安装 CLI 与正在运行的命名会话版本、私有协议及逐项功能状态。安装文件升级后旧 daemon 可能仍独立运行，因此二者必须分开核对。网站仅作只读探测，不下载安装、重启或升级 Herdr。

已固定测试 Herdr 0.8.2（协议 20）、0.9.0 和 0.9.1（协议 22）。未列入测试的产品版本若协议与能力一致，不仅因版本号而拒绝；未知私有协议或明确的 CLI/daemon 协议错配禁用相应终端功能，仍保留可用的 JSON 快照。诊断暂时超时显示未知，重新检查会重试，不永久标成不支持。主机重连或 live 版本变化会重做探测，旧尺寸控制必须重新申请。

旧 Tailcat 受控端可能不支持新增的 CLI 只读诊断，此项显示未知，已适配的观察、输入与滚轮仍可使用。完整 CLI 诊断需要包含新只读接口的受控端版本；仅更新网站或重复安装同一旧版本不会补齐此能力。以已发布 CLI 的支持说明为准，网站不会自动更新受控端或 Herdr。
