...
电脑默认“自动”，手机默认“固定字号”。默认连接只观察远端画面；网页尺寸、字号、缩放、横竖屏及软键盘变化不会改动远端行列数。字号与本地显示方式会保存，远端尺寸控制不作为持久偏好恢复。

需要远端程序按网页宽度重新换行时，在当前页面选择“自适应 · 调整远端尺寸”。该模式修改同一 pane 共享的真实尺寸，原生 Herdr 窗口可能换行或裁切；刷新页面后不会重新启用。切换回“自动”“适应窗口”或“固定字号”释放控制；若已有裁切，退出其他网页的自适应模式，再调整一次原生终端窗口大小，无需重启任务。

提示“此终端正在其他窗口自适应显示”时，关闭另一个控制窗口后重连，或切换为本地显示方式继续查看。此保护只针对另一条直接控制连接，不意味着原生 Herdr 布局不会受到自适应影响。
