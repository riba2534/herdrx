...
图片输入无需额外服务。语音默认关闭，需管理员配置 OpenAI Realtime 兼容网关，并为浏览器提供 HTTPS 或 localhost；网关密钥通过服务器受限环境文件注入。安装与上限配置见 [语音输入与图片](voice-input.md)。
...
- 适配的 Herdr 私有协议为 20 和 22；固定真实实例回归覆盖 Herdr 0.8.2（20）、0.9.0 和 0.9.1（22）。其他版本及不能准确还原的像素尺寸会明确报错，不尝试猜测输入协议或改变尺寸。
...
网站和原生终端查看同一 pane 时，默认保留原生终端尺寸。电脑可用“自动”或“适应窗口”缩放查看，手机默认以固定字号滑动查看。终端工具中的“使用此窗口尺寸”是当前页面的主动选择，远程确认后才取得控制，会修改共享尺寸并影响原生窗口；可明确释放或交接给本站其他窗口。该选择不在刷新或回到前台后恢复。排查裁切见 [运维说明](operations.md#网页与原生终端同时使用)。
